...
from meshclaude.messages.app_messages import ConnectionEstablished, ConnectionLost
from meshclaude.transport.ble_transport import BLETransport
from meshclaude.transport.discovery import scan_ble_devices, scan_serial_ports
from meshclaude.transport.serial_transport import SerialTransport
from meshclaude.transport.tcp_transport import TCPTransport
...
class ConnectionScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="conn-container"):
            yield Label("MeshClaude — Connect to Radio", classes="section-label")
    ...
    def _scan_serial(self) -> None:
        ...
        self.call_from_thread(self._populate_serial_table, ports)
    ...
    def _connect_worker(self, transport) -> None:
        try:
            ...
            if remember:
                ...
                from meshclaude.config.settings import save_config
                save_config(cfg)
            self.call_from_thread(self._on_connect_success)
        except Exception as exc:
            self.call_from_thread(self._on_connect_failure, str(exc))
